...
export default function ParentDashboard() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[#2e6b3a]" />
      </div>
    );
  }
  ...
  return (
    <div className="p-4 md:p-8 bg-gray-50 min-h-screen">
      {/* Welcome Header */}
      <div className="relative overflow-hidden rounded-[2.5rem] p-8 md:p-12 mb-8 shadow-2xl shadow-[#1a3f22]/20" style={{ background: "linear-gradient(135deg, #1a3f22, #2e6b3a)" }}>
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-white mb-2 tracking-tight">
              Hello, {user?.name || "Parent"}! 👋
            </h1>
            <p className="text-[#e6f0e8] text-lg md:text-xl opacity-90 font-medium">
          ...
          <div className="hidden lg:flex items-center gap-4 bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-3xl">
            <div className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center shadow-lg">
              <ShieldCheck className="w-6 h-6 text-[#2e6b3a]" />
            </div>
            <div>
              <p className="text-white font-bold">Verified Account</p>
              <p className="text-[#c8ddc9] text-xs">Access all parent features</p>
            </div>
          </div>
        </div>
        {/* Abstract Background Shapes */}
        <div className="absolute top-0 right-0 -mr-16 -mt-16 w-64 h-64 bg-white/10 rounded-full blur-3xl" />
        <div className="absolute bottom-0 left-0 -ml-16 -mb-16 w-64 h-64 bg-[#8DC63F]/20 rounded-full blur-3xl" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <h2 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
            <Clock className="w-5 h-5 text-[#2e6b3a]" />
            Quick Access
          </h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {quickActions.map((action, i) => (
              <Link key={i} href={action.href}>
                <div className="group bg-white rounded-3xl p-6 border border-gray-100 shadow-sm hover:border-[#8DC63F] hover:shadow-xl hover:shadow-[#e6f0e8] transition-all text-center cursor-pointer h-full flex flex-col items-center justify-center gap-3">
                  ...
                  <span className="font-bold text-gray-700 group-hover:text-[#2e6b3a] transition-colors text-sm">{action.label}</span>
                  <div className="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center group-hover:bg-[#2e6b3a] transition-colors">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-[2rem] p-6 border border-gray-100 shadow-sm overflow-hidden relative group">
            ...
            <Link href="/parent-dashboard/contact-school">
              <button className="w-full py-3 bg-[#1a3f22] text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 hover:bg-[#2e6b3a] transition-colors">
          ...
          {/* Banner/Ad-like card */}
          <div className="rounded-[2rem] p-6 text-white shadow-lg shadow-[#8DC63F]/20" style={{ background: "linear-gradient(135deg, #477023, #8DA750)" }}>
            <h3 className="font-bold text-lg mb-2">Academic Calendar</h3>
            <p className="text-[#f0f5e9] text-sm opacity-90 mb-4">
